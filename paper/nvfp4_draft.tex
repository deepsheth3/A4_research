...
\title{Microscaling Redundancy: Two Corrections Suffice for\\
NVFP4 Post-Training Quantization, with Application to\\
4-bit Draft Models for Speculative Decoding}
...
\begin{abstract}
NVFP4, the E2M1 grid with a per-16-element FP8 block scale, is the native 4-bit
format of Blackwell GPUs, and a substantial literature attempts to close its accuracy
gap to FP8 using input rotations, channel permutations, salient-channel scaling,
coordinated rounding, and per-layer rank allocation. We study why many of these give
little marginal benefit once the format already applies a per-block scale. Under the
microscaling quantizer with a high-resolution white-error approximation, the output
distortion of a linear layer is well described by a sum over blocks whose only
adjustable quantities are the per-block input maxima and the residual subspace. This
suggests two first-order levers: per-channel equalization, which lowers the block
maxima, and an additive low-rank correction of the quantization residual. We show
empirically in a post-training setting that after equalization the additive correction
provides substantially more improvement than the remaining multiplicative transforms;
a rotation still contributes a small residual gain, several times smaller than the
additive correction, which is consistent with equalization moving each block toward
the Gaussian regime under which a rotation is close to neutral. A codec built from the two levers reaches 6.05 perplexity with 4-bit
activations on Llama-3.1-8B, against 5.948 for FP8, and 6.36 with 4-bit weights,
activations, and KV cache simultaneously, with weight storage below one byte per
parameter under our side-channel accounting. We then study
the quantized model as a draft for speculative decoding, where the target model
verifies every token, so quantization loss reduces the acceptance rate rather than the
output quality. The 4-bit draft accepts 88.9\% of tokens against the FP16 target,
which corresponds to about three accepted draft tokens and four emitted tokens per
verification step. Finally, we report measured Blackwell throughput showing that the
speed advantage of NVFP4 over FP8 appears only at large batch size, and that at
single-stream decoding the two formats are within ten percent, so a 4-bit method of
this kind is best evaluated on accuracy and acceptance per byte rather than on
single-stream speed.
\end{abstract}
...
We make four contributions. First, we give an approximate distortion model for the
microscaling quantizer and a proposition identifying two first-order levers,
equalization and additive low-rank correction, together with empirical evidence that,
in our post-training setting after equalization, several rotation, permutation,
scale-objective, and coordinated-rounding variants we tested contribute little beyond
these two operations (Section~\ref{sec:theory}). Second, we build a codec from these two
operations (Section~\ref{sec:method}). Third, we evaluate it on perplexity, ablations
that the model predicts, four models, the composition of 4-bit weights, activations,
and KV cache, and a draft-model acceptance measurement (Section~\ref{sec:exp}). Fourth,
we report measured Blackwell throughput and discuss the resulting positioning of the
method (Section~\ref{sec:limits}). We are explicit throughout about which claims rest
on the approximation and which experiments remain to be run.
...
Section~\ref{sec:exp}. This is an empirical regularity under our settings, not a claim
that rotations cannot help in other regimes; learned rotations have been reported to
help in related settings \cite{liu2024spinquant}.
\end{observation}
...
We state Proposition~\ref{prop:main} as a description of the approximate model together
with an empirical claim about our setting, not as an optimality theorem. We do not claim
that rotation is useless after equalization; we measure a small residual benefit from
rotation that is consistent with reports that learned rotations help
\cite{liu2024spinquant}, and we find that additive correction dominates it. Recent
...
\subsection{4-bit draft model for speculative decoding}
When the quantized model is used as a draft, the target model verifies each proposed
token, so the output distribution is that of the target and quantization loss reduces
the acceptance rate rather than the output quality. Measured over 9{,}207 tokens against
...
weights. Closing it would require trained correction of the activation path, where the
residual is close to white, which we leave to future work.
...
the same eval. Until those are run we do not claim a best-in-class result, only the
...
hardware-native execution path. For speculative decoding we use the two-model
formulation \cite{leviathan2023speculative}; Medusa \cite{cai2024medusa} and EAGLE
\cite{li2024eagle} instead train feature-level draft heads, whereas our draft is a
standalone quantized model, so we report acceptance, the quantity that governs the
speedup of these schemes.
...
multiplicative and rounding refinements add little after equalization. A codec built
from these operations is accurate enough to serve as a draft for speculative decoding,
with 88.9\% acceptance against the FP16 target. Because measured throughput shows no
single-stream speed advantage for NVFP4 at this scale, we suggest that 4-bit methods of
this kind be compared on accuracy and acceptance per byte and on memory footprint, and
we identify the equal-byte baseline study and trained activation-path correction as the
work needed to turn these measurements into a defensible comparison.
